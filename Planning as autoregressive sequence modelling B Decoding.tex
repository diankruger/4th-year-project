...
with a beam width of $3$ performing slightly worse than the other two strategies in most layouts.
The three decoding strategies also yielded similar average steps to goal across the eight maze layouts.
...
performance degraded at a significanlt faster rate than the decoding strategies as the maze layouts
became more complex.

The results indicate that predicting the outcomes of more candidate trajectories did not provide
...
outperformed Grid-Bug2 in the more complex maze layouts indicates that the model is providing some useful 
guidance for action selection, and that the decoding strategies are able to leverage this guidance to 
outperform a deterministic baseline.
