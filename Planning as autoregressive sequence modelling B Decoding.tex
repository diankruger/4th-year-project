...
objective, this amounts to using imitation learning to solve the planning problem.gi
...
The objective of this experiment is to evaluate different decoding strategies
when training a conditional trajectory model through next-token prediction in a partially
observed maze environment. Instead of being provided with the maze 
layout and the exact coordinates of the agent at every step,
the model is instead provided with a 3x3 grid of cells surrounding the current position. Since there 
are no coordinates provided to use as reference, the model recieves the relative 
displacement of the goal from the current position at every step, as opposed to the absolute 
coordinates of the goal. The model therefore would need to rely on previously observed trajectory information to infer
the maze layout and its current position in the maze. This is meant to showcase one advantage of 
sequence modelling, the model naturally has a memory of the previous trajectory prefix, 
which can be used to infer unobserved information about the environment.

At inference time, the same decoding algorithms as the previous experiment are
then used to construct complete trajectories by selecting continuations with high probability 
under the learned model. Simialrly to the previous experiment, since decoding is based on 
model probability rather than a reward-maximising, making this an imitation learning problem.
...
since the exact position in the maze after every action is known. In the partially oberved setting, however,
the position in the maze after every action is not exactly known, 
so the observations that follows from each action need to be predicted. 
Supervision during training is thus applied to the observations as well as the actions in 
the sequences. 

Each trajectory $\boldsymbol{\tau} = (z_1, z_2, \dots, z_{2T})$ thus consists of alternating state and observation 
tokens, \\ $(o_1, a_1, \Delta g_1, o_2, a_2, \Delta g_2, \dots, o_T, a_T, \Delta g_T)$, where each observation $o_t$ denotes the 3x3 grid of cells surrounding the current position,
the actions $(a_1, a_2, \dots, a_T)$ corresponds to the moves (up, down, left, or right) between consecutive positions, and 
\begin{equation}
\Delta \mathbf{g}_t
=
\begin{pmatrix}
...
\end{pmatrix}.
\end{equation}
denoting the relative displacement of the goal from the current position, where $(r_t, c_t)$ denote the coordinates of the current position and $(r_{\mathrm{goal}}, c_{\mathrm{goal}})$ denote the coordinates of the goal.  

 
Let $o_{\mathrm{start}}$ denote the initial observation.The model therefore learns conditional predictions of the form
\begin{equation}
p_{\hat{\boldsymbol{\theta}}}(z_{t+1} \mid o_{\mathrm{start}}, s_{\mathrm{goal}}, z_{\le t}),
\end{equation}
or, since supervision is applied only at action-token positions, equivalently
\begin{equation}
p_{\hat{\boldsymbol{\theta}}}(a_{t} \mid m, s_{\mathrm{start}}, s_{\mathrm{goal}}, s_1, a_1, \dots, s_t),
