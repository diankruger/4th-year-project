...
distribution, the model learns a specific family of distributions parameterised by $\theta$ \cite{goodfellow2016deep}. 

If the data's true distribution $p(\mathbf{x})$ is assumed to belong to a family of distributions
$\{p_\theta(\mathbf{x})\}$, the model must learn the parameter value $\hat{\theta}$ such that
$p_{\hat{\theta}}(\mathbf{x})$ is a good approximation of $p(\mathbf{x})$.

One way to formalise this is through the Kullback--Leibler (KL) divergence, which measures how one
probability distribution diverges from another. It is defined as
\begin{equation}
    D_{\mathrm{KL}}(p(\mathbf{x}) \| p_\theta(\mathbf{x})) = \mathbb{E}_{\mathbf{x} \sim p(\mathbf{x})} \left[ \log \frac{p(\mathbf{x})}{p_\theta(\mathbf{x})} \right] \cite{goodfellow2016deep}.
\end{equation}
In this setting, learning can be viewed as choosing $\theta$ to minimise
$D_{\mathrm{KL}}(p(\mathbf{x}) \| p_\theta(\mathbf{x}))$. Expanding this expression gives
\begin{equation}
    D_{\mathrm{KL}}(p(\mathbf{x}) \| p_\theta(\mathbf{x}))
    =
    \mathbb{E}_{\mathbf{x} \sim p(\mathbf{x})} [\log p(\mathbf{x})]
    -
    \mathbb{E}_{\mathbf{x} \sim p(\mathbf{x})} [\log p_\theta(\mathbf{x})].
\end{equation}
The first term does not depend on $\theta$, so minimising the KL divergence is equivalent to
maximising $\mathbb{E}_{\mathbf{x} \sim p(\mathbf{x})} [\log p_\theta(\mathbf{x})]$, that is, the
expected log-likelihood of the data under the model \cite{goodfellow2016deep}.

The process of finding $\hat{\theta}$ that maximises the likelihood of the training data under
$p_\theta(\mathbf{x})$ is known as maximum likelihood estimation (MLE), and is a common approach to
...
Imitation learning is a related paradigm in which a model learns to reproduce the behavior
...
\]
where $s_t$ is the state (coordinates) at time step $t$ and $a_t$ is the action (moving up, down, left or right) taken at time step $t$, the trained generative model defines an approximation $p_{\hat{\theta}}(\boldsymbol{\tau})$ to the distribution over valid trajectories.
Instead of directly modelling $p_{\hat{\theta}}(\boldsymbol{\tau})$, an autoregressive model factorises this distribution into a product of conditional probabilities \cite{goodfellow2016deep}:
\[
p_{\hat{\theta}}(\boldsymbol{\tau}) = \prod_{t=1}^{T} p_{\hat{\theta}}(z_t \mid z_{<t})
\]
...
This simplifies the supervised learning problem to estimating $p_\theta(z_t \mid z_{<t})$, so given a prefix 
...
Direct sampling from $p_{\hat{\theta}}(\boldsymbol{\tau})$ is challenging, since the number of possible trajectories grows exponentially with trajectory
...
This creates a need for decoding algorithms to approximately sample from, or search within $p_{\hat{\theta}}(\boldsymbol{\tau})$.



\subsubsection*{Greedy Decoding}
Greedy decoding is the simplest method for generating sequences from $p_{\hat{\theta}}(\boldsymbol{\tau})$ \cite{jurafsky2026speech}. 
At each step $t$ during the autoregressive generation of a trajectory, the algorithm selects the next token $\hat{z}_t$ 
as follows:
\[
\hat{z}_t = \arg\max_{z \in V} p_{\hat{\theta}}(z \mid z_{<t})
\]
...
$p_{\hat{\theta}}(\boldsymbol{\tau})$ \cite{jurafsky2026speech}.
...
6: & \hspace*{1.5em}$\mathcal{T}_{t+1} \leftarrow$ the set of the $B$ highest-scoring sequences in $C_{t+1}$ under $\log p_{\hat{\theta}}(\boldsymbol{\tau})$ \\[0.3em]
...
9: & \textbf{Return} $\arg\max_{\boldsymbol{\tau} \in \mathcal{T}_{t+1}} \log p_{\hat{\theta}}(\boldsymbol{\tau})$
\end{tabular}
...
\end{figure}
The score $\log p_{\hat{\theta}}(\boldsymbol{\tau})$ can be accumulated autoregressively for each candidate trajectory. 
...
\begin{itemize}
\item \textbf{Output Projection}: Let $\mathbf{h}_t$ denote the final hidden representation produced after
...
\[
p_{\hat{\theta}}(z_{t+1} \mid m, s_{\mathrm{start}}, s_{\mathrm{goal}}, z_{\le t}),
\]
or, since supervision is applied only at action-token positions, equivalently
\[
p_{\hat{\theta}}(a_{t} \mid m, s_{\mathrm{start}}, s_{\mathrm{goal}}, s_1, a_1, \dots, s_t),
\]
...
\[
p_{\hat{\theta}}(a_1, a_2, \dots, a_T \mid m, s_{\mathrm{start}}, s_{\mathrm{goal}})
=
\prod_{t=1}^{T}
p_{\hat{\theta}}(a_t \mid m, s_{\mathrm{start}}, s_{\mathrm{goal}}, s_1, a_1, \dots, s_t).
\]
...
left, and right, with each assigned its own unique integer token identifier.
...
In addition to the input matrix $X$, the tokenised input sequences are stored in an input matrix $X$, 
...
at $19$ steps for all queries. This value was chosen because the Transformer was trained with a 
maximum input sequence length of $42$ tokens, while the initial decoding prefix already occupies four 
tokens: the maze token, the start state token, the goal state token, and the initial state token. 
Each subsequent decoding step appends one action token and one next-state token, so the largest 
number of decoding steps that can be represented without exceeding the model's maximum sequence 
length is
\[
\left\lfloor \frac{42 - 4}{2} \right\rfloor = 19.
\]
The fixed step limit of $19$ therefore ensures that decoding remains consistent with the Transformer's 
input size while allowing the longest rollout permitted by the model.
...
The following table reports the overall decoding performance across all eight maze layouts, 
aggregated over the full cross-validation evaluation.

\begin{table}[H]
\centering
\caption{Dataset-wide decoding results aggregated across all eight maze layouts.}
\label{tab:decoding_results_all8}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lcccc}
\hline
Strategy & Completion Rate & Avg. Steps to Goal & Avg. Compute Time (s) & Avg. Time per Step (s) \\
\hline
Greedy   & 0.6916 & 7.7205 & 0.01766 & 0.00155 \\
Beam 2   & 0.9100 & 7.0771 & 0.01401 & 0.00167 \\
Beam 3   & 0.9754 & 6.9058 & 0.01298 & 0.00173 \\
\hline
\end{tabular}%
}
\end{table}

Across all five cross-validation folds, the evaluation set contained 
5976 trajectory records in total, distributed as follows: 
OPEN (240), U\_MAZE (90), U\_MAZE\_EVAL (90), SMALL\_MAZE (24), MEDIUM\_MAZE (650), 
MEDIUM\_MAZE\_EVAL (650), \\ LARGE\_MAZE (2070), and LARGE\_MAZE\_EVAL (2162). 

Larger mazes consequently contribute more records to the overall evaluation, so the
aggregated results in Table~\ref{tab:decoding_results_all8} are dominated by the performance 
on the larger maze layouts. Appendix~\ref{app:per_maze_results} reports the performance of the 
decoding strategies for each individual maze layout.

\section*{Discussion and Conclusion}

It is observed in Table~\ref{tab:decoding_results_all8} that beam search with width $B=2$ and
$B=3$ outperforms greedy decoding in terms of both completion rate and average steps to goal,
...
increasing beam width, is that the broader search requires more computation. The average time per
step shown in Table~\ref{tab:decoding_results_all8} is indeed higher for beam search than for
greedy decoding, and in the aggregate it also increases with beam width. However, in this
implementation the average total compute time per query is lower for beam search than for greedy
...
In conclusion, the results demonstrate that beam search is a more effective decoding strategy than
greedy decoding for the maze planning problem under the learned trajectory model, both in terms of
...
The current work has focused on comparing decoding strategies, specifically greedy decoding and beam
search, when solving the maze planning problem through imitation learning under a learned trajectory
model. One underlying assumption is that the model has been trained on optimal trajectories, so that
